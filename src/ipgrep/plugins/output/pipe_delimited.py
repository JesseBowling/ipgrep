...
class PipeDelimitedOutput(OutputPlugin):
    """Output plugin that formats IPs as pipe-delimited text.

    Each line contains all fields for an IP, separated by pipes (|).
    """
    ...
    def format(self, ip_data_list: List[Dict[str, Any]]) -> str:
        ...
        Returns:
            Pipe-delimited output with one IP per line.
        """
        if not ip_data_list:
            return ""
